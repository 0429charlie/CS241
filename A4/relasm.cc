...
void readid(vector<Token *> v, vector<pair<string, int> *> *pr, int index, vector<int *> *relocate) {
	...
	if (v[index]->getLexeme() == "jr") {
		...
		if (pasttwo == 1) {
			...
			// get the current address
                	int addr = (4*nofintr)+0xc;
                	// allocate the memory for the new entry in the relocation table
                	int *address = new int;
			*address = addr;
			int *form = new int;
			int formate = 0x01;
			*form = formate;
                	relocate->push_back(form);
			relocate->push_back(address);	
		}
        }
	return;
}
...
// vector<Token *> (vector<pair<string, int>> *) Int -> Void
void readlabel(vector<Token *> v, vector<pair<string, int> *> *pr, int idx, vector<int *> *relocate) {
	...
	if (v.size() > (idx+1)) {
		...
		if (v[j]->getKind() == DOTWORD) {
			readword(v, j, pr, relocate);
		} else if (v[j]->getKind() == LABEL) {
			readlabel(v, pr, j, relocate);
		} else if (v[j]->getKind() == ID) {
			readid(v, pr, j, relocate);
		} else {
...
int main(int argc, char* argv[]){
	...
	// allocate a space for symbol table
        vector<pair<string, int> *> *prt = new vector<pair<string, int> *>;

	// allocate a space for relocation table
	vector<int *> *relocate = new vector<int *>;
...
//-------------------------------------------------------------------
        ...
        for (int i = 0; i < tokLines.size(); i++) {
                ...
                if (token.size() != 0) {
                        if (token[0]->getKind() == DOTWORD) {
                                readword(token, 0, prt, relocate);
                        } else if (token[0]->getKind() == LABEL) {
                                readlabel(token, prt, 0, relocate);
                        } else if (token[0]->getKind() == ID) {
                                readid(token, prt, 0, relocate);
                        } else {
                                throw string("ERROR: Invalid label");
                        }
                }
	...
	// out put the first line of the .merl
	int h1 = 0x10000002;
	putchar(h1>>24);
	putchar(h1>>16);
	putchar(h1>>8);
	...
	// out put the second line of the .merl
	int h2 = (3+nofintr+relocate->size())*4;
	putchar(h2>>24);
        putchar(h2>>16);
        putchar(h2>>8);
        putchar(h2);

	// out put the thirst line of the .merl
	int h3 = (3+nofintr)*4;
	putchar(h3>>24);
        putchar(h3>>16);
        putchar(h3>>8);
...
//-------------------------------------------------------------------	
	...
	// print and delete the allocated relocation table
	for (int l = 0; l < relocate->size(); l++) {
		int rel = *((*relocate)[l]);
		putchar(rel>>24);
        	putchar(rel>>16);
        	putchar(rel>>8);
